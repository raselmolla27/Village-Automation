...
/* Fireflies */
struct FF { float x,y,ph,rad,spd; };
FF ffly[22];
bool fflyOn = false;
...
void initParticles(){
    for(int i=0;i<70;i++){
        ...
        rain[i].y=(float)(650+rand()%200);
        rain[i].spd=4.0f+(rand()%30)/10.0f;
        rain[i].len=8.0f+(rand()%14);
    }
    for(int i=0;i<22;i++){
        ffly[i].x=(float)(50+rand()%1000);
        ffly[i].y=(float)(180+rand()%320);
        ffly[i].ph=(rand()%628)/100.0f;
        ffly[i].rad=(float)(22+rand()%55);
        ffly[i].spd=0.01f+(rand()%20)/1000.0f;
    }
}
...
void drawCar(float cx,float cy,float roadAng){
    ...
    /* Rotate so car faces along the road direction */
...
/* ===============================================================
   VILLAGER - walks on LEFT side of road, faces walking direction
   =============================================================== */
void drawVillager(float vx,float vy,float roadAng){
    glPushMatrix();
    glTranslatef(vx,vy,0);
    /* roadAng aligns body along road; villager walks upward */
...
void drawRain(){
    if(!rainOn) return;
    glColor4f(0.50f,0.65f,0.92f,0.48f);
    glLineWidth(1.2f);
    for(int i=0;i<70;i++)
        ddaLine(rain[i].x,rain[i].y,rain[i].x-2,rain[i].y-rain[i].len);
    glLineWidth(1.0f);
}
...
void drawFireflies(){
    if(!fflyOn) return;
    for(int i=0;i<22;i++){
        float bk=0.3f+0.7f*fabsf(sinf(ffly[i].ph));
        float fx=ffly[i].x+ffly[i].rad*sinf(ffly[i].ph*0.7f);
        float fy=ffly[i].y+ffly[i].rad*0.5f*cosf(ffly[i].ph*0.9f);
        glColor4f(0.95f,0.92f,0.30f,bk*0.14f); fc(fx,fy,13);
        glColor4f(1.0f,0.98f,0.50f,bk);        fc(fx,fy,3);
        glColor4f(0.95f,0.90f,0.20f,bk*0.38f);
        glPointSize(1.0f); midpointCircle((int)fx,(int)fy,7);
    }
}
...
void display(){
    ...
    /* FIX: Right hut moved away from road to x=790 */
    ...
    /* FIX: Villager walks on left side of road (offset -35 from center),
       and uses separate t-range so car doesn't run over them */
    {
        float x,y;
    ...
    drawText(10,30,"Left Click=Zoom In  Arrow Keys=Explore  P=Pause  1=Rain  2=Fireflies  F=Fullscreen  R=Reset  ESC=Quit");
...
void update(int){
    ...
    if(!paused){
        ...
        if(fflyOn){
            for(int i=0;i<22;i++) ffly[i].ph += ffly[i].spd;
        }
    }
...
void createMenu(){
    ...
    glutAddMenuEntry("Toggle Fireflies  (2)",2);
